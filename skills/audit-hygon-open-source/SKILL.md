...
7. When the user requests exact complete-history counts, Commit title/body review, or history-rewrite pre/post evidence, additionally run the bundled read-only history mode:
   ...
   Reject shallow repositories. Interpret `sugon/rogon` as blockers and `dcu` as a migration review item, not an automatic violation.
...
- Under policy v1.3 and later, block case-insensitive `sugon` and `rogon` occurrences in committed file paths, committed text, and private Commit author, committer, email, subject, or body metadata. Allow `hygon`, including HYGON email addresses.
...
- Never perform HCU/DCU naming or AMD/XGMI runtime wording remediation in this Skill. Use `$audit-hygon-platform` as the sole platform-specialist Skill.
